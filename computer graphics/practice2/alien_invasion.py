...
class AlienInvasion:
    def __init__(self):
        ...
        self.alien_explosion_sound = pygame.mixer.Sound('alien_explosion.mp3')
    ...
    def _fire_bullet(self):
        if len(self.bullets) < self.settings.bullets_allowed:
            new_bullet = Bullet(self)
            self.bullets.add(new_bullet)
            self.laser_sound.play()  # Play laser sound when a bullet is fired
    ...
    def _check_bullet_alien_collisions(self):
        ...
        if collisions:
            # Добавляем 10 очков за каждого уничтоженного инопланетянина
            self.score += 10 * len(collisions)
            self.alien_explosion_sound.play()  # Звук взрыва инопланетянина
        if not self.aliens:  # Если все инопланетяне уничтожены
            self.bullets.empty()  
            self.settings.increase_speed() 
            self._create_fleet()  # Создаём новый флот инопланетян
