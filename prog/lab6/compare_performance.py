...
for num in TEST_LST[:5]:  # Проверим на первых 5 числах
    ...
    cy_result = cy_factorization.fermat_factorization(num)
...
try:
    cy_times = timeit.repeat(
        "res = [cy_factorization.fermat_factorization(i) for i in TEST_LST]",
        setup='from __main__ import cy_factorization, TEST_LST',
        number=NUMBER,
        repeat=REPEAT
    )
